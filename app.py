...
def admin_dashboard():
    ...
    # Get all books with relationships loaded
    all_books = Book.query.options(
...
def borrow_book(book_id):
    if 'user_id' not in session:
        # Store the book ID in session for after login
...
def edit_book(book_id):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('view_shelf', shelf_id=book.rack.shelf_id))
